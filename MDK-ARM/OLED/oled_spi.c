...
{ 
	...
	OLED_WR_Byte_spi((x&0x0f)|0x01,OLED_CMD); 
}   	  
//开启OLED显示    
...
{  
	...
	{  
		...
		OLED_WR_Byte_spi (0x00,OLED_CMD);      //设置显示位置—列低地址
...
{      	
	unsigned char c=0,i=0;	
		...
		if(SIZE_SPI ==16)
			...
			for(i=0;i<8;i++)
			OLED_WR_Byte_spi(F8X16[c*16+i],OLED_DATA);
			OLED_Set_Pos_spi(x,y+1);
			for(i=0;i<8;i++)
			OLED_WR_Byte_spi(F8X16[c*16+i+8],OLED_DATA);
			}
			else {	
				OLED_Set_Pos_spi(x,y+1);
				for(i=0;i<6;i++)
				OLED_WR_Byte_spi(F6x8[c][i],OLED_DATA);
...
//显示一个字符号串
void OLED_ShowString_spi(u8 x,u8 y,u8 *chr)
{
	unsigned char j=0;
	while (chr[j]!='\0')
...
{ 	
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6|GPIO_Pin_12|GPIO_Pin_7;	 
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;//速度50MHz
 	GPIO_Init(GPIOA, &GPIO_InitStructure);	  //初始化GPIOD3,6
 	GPIO_SetBits(GPIOA,GPIO_Pin_6|GPIO_Pin_7|GPIO_Pin_12);	
  ...
  OLED_RST_Set();
	...
	OLED_WR_Byte_spi(0x00,OLED_CMD);//---set low column address
	...
	OLED_WR_Byte_spi(0xCF,OLED_CMD); // Set SEG Output Current Brightness
