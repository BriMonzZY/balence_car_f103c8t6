...
void oled_show(void)
{
		if(pitch<0)		
		{
			...
			OLED_Float(0,56,-eulerAngle_mpu6050.pitch,2);
		}	
		else	
		{
			OLED_ShowString(48,0,"+",12); 
			OLED_Float(0,56,eulerAngle_mpu6050.pitch,3);			
		}
		OLED_Num2(4,6, Right_encoder);					//显示右边电机的编码器值
		OLED_Num2(14,6, Left_encoder);					//显示左边电机的编码器值
		//OLED_Float(4,70, UltrasonicWave_Distance, 2);						//显示超声波的距离
